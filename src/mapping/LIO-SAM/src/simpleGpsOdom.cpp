...
public:
    GNSSOdom() : nh("~") {
        ...
        // nh.param<std::string>("lio_sam_6axis/gpsFixTopic", gpsFixTopic, "navsat/fix");

        std::cout << "gpsFixTopic: " << std::endl;

        gpsSub = nh.subscribe(gpsFixTopic, 100, &GNSSOdom::GNSSCB, this, ros::TransportHints().tcpNoDelay());
