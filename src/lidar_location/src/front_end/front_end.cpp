...
bool FrontEnd::InitWithConfig() {
    std::string config_file_path = ros::package::getPath("lidar_location") + "/config/front_end/config.yaml";
...
bool FrontEnd::InitDataPath(const YAML::Node& config_node) {
    ...
    if (data_path_ == "./") {
        data_path_ = ros::package::getPath("lidar_location");
    }
    ...
    if (!boost::filesystem::is_directory(data_path_)) {
        ROS_WARN_STREAM("文件夹 " << data_path_ << " 未创建成功!";);
        return false;
    } else {
        ROS_INFO_STREAM("地图点云存放地址：" << data_path_;);
    }
    ...
    if (!boost::filesystem::is_directory(key_frame_path)) {
        ROS_WARN_STREAM("文件夹 " << key_frame_path << " 未创建成功!";);
        return false;
    } else {
        ROS_INFO_STREAM("关键帧点云存放地址：" << key_frame_path << std::endl << std::endl;);
    }
...
bool FrontEnd::InitRegistration(std::shared_ptr<RegistrationInterface>& registration_ptr, const YAML::Node& config_node) {
    ...
    ROS_INFO_STREAM("点云匹配方式为：" << registration_method;);

    if (registration_method == "NDT") {
        registration_ptr = std::make_shared<NDTRegistration>(config_node[registration_method]);
    } else {
        ROS_ERROR_STREAM("没找到与 " << registration_method << " 相对应的点云匹配方式!";);
        return false;
    }
...
bool FrontEnd::InitFilter(std::string filter_user, std::shared_ptr<CloudFilterInterface>& filter_ptr, const YAML::Node& config_node) {
    ...
    ROS_INFO_STREAM(filter_user << "选择的滤波方法为：" << filter_mothod;);

    if (filter_mothod == "voxel_filter") {
        filter_ptr = std::make_shared<VoxelFilter>(config_node[filter_mothod][filter_user]);
    } else {
        ROS_ERROR_STREAM("没有为 " << filter_user << " 找到与 " << filter_mothod << " 相对应的滤波方法!";);
        return false;
    }
